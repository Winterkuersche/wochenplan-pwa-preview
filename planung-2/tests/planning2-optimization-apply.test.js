...
test('accepted full-day suggestion gets pause and work minutes exclusively from central time rules',()=>{for(const proposedStart of ['08:55','09:00'])for(const proposedEnd of ['19:00','19:10']){const suggestion={...base,proposedStart,proposedEnd},plan={schedule:{[isoDate]:{anna:shift}},absences:[]},h=harness(plan,suggestion);assert.equal(h.api.applyPlanning2OptimizationSuggestion(suggestion),true,`${proposedStart}–${proposedEnd}`);const result=h.state().schedule[isoDate].anna,expectedPause=centralTime.getBusinessRequiredBreakMinutes(proposedStart,proposedEnd);assert.equal(result.pause,expectedPause,`${proposedStart}–${proposedEnd} pause`);assert.equal(result.breakMinutes,expectedPause,`${proposedStart}–${proposedEnd} breakMinutes`);assert.equal(result.minutes,centralTime.getWorkedMinutesFromRange(proposedStart,proposedEnd,expectedPause),`${proposedStart}–${proposedEnd} minutes`);assert.equal(result.withCheckout,proposedEnd==='19:10',`${proposedStart}–${proposedEnd} withCheckout`)}});
