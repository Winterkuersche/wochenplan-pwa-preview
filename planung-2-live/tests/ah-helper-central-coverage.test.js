...
test('zentraler AH-Helper rendert branch + minutes deterministisch', () => {
  ...
  assert.equal(
    html,
    '<span class="ahCellContent">  <span class="ahCellMain">AH</span>  <span class="ahCellSubrow"><span class="ahCellBranch" title="Filiale Nord">Filiale Nord</span><span class="ahCellSeparator" aria-hidden="true"> · </span><span class="ahCellDuration">3:00</span></span></span>'
  );
});

test('zentraler AH-Helper ohne Ziel bleibt stabil ohne leeren Separator', () => {
  const ctx = loadMonthHelpers();
...
test('zentraler AH-Helper ohne Minuten zeigt keine falsche Stundenanzeige', () => {
  const ctx = loadMonthHelpers();
...
test('normale Schicht (type: shift) bleibt im Monatsrenderer unverändert', () => {
  const ctx = loadMonthHelpers();
...
test('Monatsrenderer nutzt getExternalHelpCompactDisplay für AH', () => {
  const ctx = loadMonthHelpers();
  const resolved = {
    type: 'external-help',
  ...
  assert.equal(ctx.getMonthCellText(resolved), ctx.getExternalHelpCompactDisplay(resolved));
});

test('Übersichtsrenderer nutzt denselben AH-Helperpfad', () => {
